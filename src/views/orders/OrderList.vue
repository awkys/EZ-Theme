  ...
  <div class="orders-container">
    <div class="orders-inner">
      ...
      <div v-else-if="error" class="orders-error">
        ...
        <button class="retry-button" @click="fetchOrders">
          {{ $t("common.retry") || "重试" }}
        </button>
      </div>
      ...
      <div v-else-if="orders.length > 0" class="orders-content">
        ...
        >
          ...
          <transition-group
            :name="
              slideDirection === 'right' ? 'page-switch-right' : 'page-switch'
            "
          >
            <div
...
const fetchOrders = async () => {
  ...
  try {
    ...
    error.value =
      err && err.message ? err.message : t("common.networkError") || "网络错误";
...
const getStatusText = (status) => {
  return (
    statusTextMap.value[status] || `${statusTextMap.value.unknown} (${status})`
  );
};
